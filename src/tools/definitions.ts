...
export const TOOL_DEFINITIONS: Anthropic.Messages.Tool[] = [
  {
    name: 'scale_recipe',
    description:
      'Use this tool to adjust a recipe to a different number of servings. Takes a recipe and a target serving count, returns the recipe with ingredient amounts and macros recomputed. Call this when the user asks to scale, double, halve, or change the yield of a recipe.',
    input_schema: {
      type: 'object',
      properties: {
        recipe: {
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        servings: {
          type: 'number',
          description: 'Number of servings to estimates macros per'
        }
      },
